...
class RecognizeVoiceActivity
    ...
    if network
      ...
      if is_online
        if $is_first_launch.nil?
          ...
          $context.start_service(Intent.new(self, $package.GetForecastsService.java_class))
          $context.start_service(Intent.new(self, $package.GetCurrentPrecipitationService.java_class))
        end
  ...
  def start_recognize_voice(context)
    ...
    prepare_greeting_player $player_ohayo, R.raw.ohayo
    prepare_greeting_player $player_otsukare, R.raw.otsukare
